...
wd <- "C://Users//scrol//OneDrive - McMaster University//Joseph Okeme's files - Alyss//New passives calibration//Alyss New Calculations//Updated Workflow3"
setwd(wd)
...
dedup <- function(df){
  ...
  #deduplicate features with inchikey and with known name - this is to stop unnecessary data loss for the NAs which would all be marked as duplicates of each other 
...
#Function for counting number of unique features

Get_N_Features <- function(df){
  length(unique(df$Alignment_ID))
}
...
wd_khipu = "C://Users//scrol//OneDrive - McMaster University//Joseph Okeme's files - Alyss//New passives calibration//Alyss New Calculations//Updated Workflow3" #second working directory for khipu if needed (useful to separate feature tables for different modes as they are all named the same)
...
unreal_level2 <- c(13639, 12148, 19833, 8961, 10333, 11634, 7055, 10019, 12713, 12469, 17823, 9912,
                   ...
                   6809, 7440, 9622, 10335, 12468, 14390, 15113, 18676, 19831, 4447, 5059, 2145, 1321,
                   8187, 5909) #manually checked spectra that are not level 2 (obtained from an original list of potential level 2 candidates )
#Please note that feature 1321 is annotated the same as feature 1327 at level 2 confidence, as is the case for 2145 and 2147.
#For the sake of data processing, 1321 and 2145 have been marked as unreal here because they are overall lower mean abundance.
...
#Create a dataframe for the number of features at each step to make it easier to add these values to the manuscript

N_Features <- data.frame()
N_Features[1,"Before_FirstQAQC"] = Get_N_Features(feature_table_clean) #Number of features in the alignment table before any processing
...
#The following lines have been made into notes since the final khipu export is just re-imported now
...
#The kihpu output can be imported here without needing to run from line 562
...
N_Features[,"AfterBlankFilter"] = Get_N_Features(Percent_detection_filter) #Number of features remaining after removing high blanks
N_Features[,"RemovedByBlankFilter"] = N_Features[,"Before_FirstQAQC"] - N_Features[,"AfterBlankFilter"] #Number of features that were removed beacuse of blanks

#Note that a feature will only have been removed from the feature table at this stage if the blank was high for ALL samples across ALL sampler types
...
#This code was used for checking the level 2 annotations manually and updated unreal_level2; some parts have been turned into notes as they are no longer required
...
#see level 2s before deduplicating to consider other peaks as possibilities
...
#Note that the following filtering is done across ALL samples and ALL sampler types; a feature with 100% detection on PDMS but 0% on GFF or AAS will be filtered out here
#Accordingly, the results of this filtering are not used in the manuscript but can serve as an easy overview of the most detected features
...
#-------visualize annotations for detection frequency across all samplers-------
...
#To ensure that features unique to sampler types are retained, the filtering process is repeated within sampler types

Sampler_type_separate <- Percent_detection_filter

Active <- colnames(Sampler_type_separate[,grep('_A_', colnames(Sampler_type_separate))]) #get list of all AAS samples
PDMS <- colnames(Sampler_type_separate[,grep('_D_', colnames(Sampler_type_separate))]) #get list of all PDMS-Mesh samples
GFF <- colnames(Sampler_type_separate[,grep('_G_', colnames(Sampler_type_separate))]) #get list of all GFF-Mesh samples

#Perform the same detection count, frequency, and mean calculations as before, but within sampler types instead of across all sampler types
...
Separate_filter <-  Sampler_type_separate %>%
  ...
  filter(Detection_Frequency_D >= 50 | Detection_Frequency_A >= 50 |
           Detection_Frequency_G >= 50) %>% #filter for detection frequency by sampler type
  pivot_longer(cols = all_of(all_samples_deployed), names_to = 'Sample', values_to = 'Area', values_drop_na = F) %>%
...
#Deduplicating here is being done across all sampler types; this has the potential to exclude unique features
...
#Here, the annotation frame is being diverted into deduplicating separately for each sampler type to retain unique or lower abundance features

#Deduplicate within each sampler type separately
#PDMS-Mesh
Dedup_D <- Separate_annotated %>%
  filter(Detection_Frequency_D >= 50 & Days_Detected_D >= 4) %>% #filter for 50% detection
...
#Bind back into one data frame
Seperate_dedup_individ_bound <- rbind(Dedup_D, Dedup_G, Dedup_A) #combine into one

#Add the number of features now present after deduplicating
N_Features[,"After_DetFreq_Dedup"] = Get_N_Features(Seperate_dedup_individ_bound)
N_Features[,"FilterByDetDedup"] = N_Features[,"AfterBlankFilter"] - N_Features[,"After_DetFreq_Dedup"] #Calculate how many were removed by detection frequency

#Filtering by detection frequency removes more features than the blank filtering because for many features, the blank is not high across EVERY sample
#e.g., it may be >35% for all AAS samples but not the PDMS or GFF samples and therefore is retained until the feature table is filtered by detection frequency on AAS specifically

Total_Number_of_Features <- length(unique(Separate_dedup$Alignment_ID))

Accent_Palette = brewer.pal(8, "Accent") #colours for figures
Sampler_Colours = Accent_Palette[c(1,2,3)]

#Visualize as bar graphs
features_per_replicate <- Seperate_dedup_individ_bound %>%
  group_by(Sampler) %>%
  dplyr::summarise(n = n()) %>%
  ggplot(aes(x = reorder(Sampler, -n), y = n, fill = Sampler)) +
  geom_bar(stat = "identity", show.legend = FALSE, width = 0.7, alpha = 0.7)+
...
save_plot(plot = features_per_replicate, 'Figures/features_per_replicate.svg', base_width = 8, base_height = 7)

#-------venn diagram for sampler types-------
#we are comparing features detected between sampler types so use the one with deduplication within sampler types
...
#visualize as venn diagram used in Figure 2
sampler_type_ven <- ggvenn(venn_data, stroke_size = 0.7,fill_alpha = 0.7,count_column = TRUE,
                          fill_color = Sampler_Colours, set_name_size = 8, text_size = 6, show_percentage = TRUE)+ #venn diagram between sampler types
...
save_plot(plot = sampler_type_ven, 'Figures/AllVenn.svg', base_height = 8, base_width = 9)

#Venn diagrams for each confidence level; not used in final manuscript drafts

venn_l2_3 <- Seperate_dedup_individ_bound %>%
  filter(level < 4) %>%
  unstack(Alignment_ID ~ Sampler) #separate alignment ids/inchikeys present by sampler type#

level23_ven <- ggvenn(venn_l2_3, stroke_size = 0.7,fill_alpha = 0.7,count_column = TRUE,
                           fill_color = Sampler_Colours, set_name_size = 8, text_size = 6, show_percentage = TRUE)+ #venn diagram between sampler types
  theme(legend.position="none")+
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))+
  theme(legend.title = element_text(size = 20),
...
save_plot(plot = level23_ven, 'Figures/23Venn.svg', base_height = 8, base_width = 9)

venn_l4_5 <- Seperate_dedup_individ_bound %>%
  filter(level > 3) %>%
  unstack(Alignment_ID ~ Sampler) #separate alignment ids/inchikeys present by sampler type#

level45_ven <- ggvenn(venn_l4_5, stroke_size = 0.7,fill_alpha = 0.7,count_column = TRUE,
                      fill_color = Sampler_Colours, set_name_size = 8, text_size = 6, show_percentage = TRUE)+ #venn diagram between sampler types
  theme(legend.position="none")+
  labs(title = "Level 4/5 Features") +
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))+
  theme(legend.title = element_text(size = 20),
        legend.text = element_text(size = 18),
        plot.title = element_text(size = 25, hjust = 0.5),
...
save_plot(plot = level45_ven, 'Figures/45Venn.svg', base_height = 8, base_width = 9)

venn_l2 <- Seperate_dedup_individ_bound %>%
  filter(level < 3) %>%
  unstack(Alignment_ID ~ Sampler) #separate alignment ids/inchikeys present by sampler type#

level2_ven <- ggvenn(venn_l2, stroke_size = 0.7,fill_alpha = 0.7,count_column = TRUE,
                      fill_color = Sampler_Colours, set_name_size = 8, text_size = 6, show_percentage = TRUE)+ #venn diagram between sampler types
  theme(legend.position="none")+
  labs(title = "Level 2 Features") +
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))+
  theme(legend.title = element_text(size = 20),
        legend.text = element_text(size = 18),
        plot.title = element_text(size = 25, hjust = 0.5),
...
save_plot(plot = level2_ven, 'Figures/2Venn.svg', base_height = 8, base_width = 9)

venn_l3 <- Seperate_dedup_individ_bound %>%
  filter(level == 3) %>%
  unstack(Alignment_ID ~ Sampler) #separate alignment ids/inchikeys present by sampler type#

level3_ven <- ggvenn(venn_l3, stroke_size = 0.7,fill_alpha = 0.7,count_column = TRUE,
                     fill_color = Sampler_Colours, set_name_size = 8, text_size = 6, show_percentage = TRUE)+ #venn diagram between sampler types
  theme(legend.position="none")+
  labs(title = "Level 3 Features") +
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))+
  theme(legend.title = element_text(size = 20),
        legend.text = element_text(size = 18),
        plot.title = element_text(size = 25, hjust = 0.5),
...
save_plot(plot = level3_ven, 'Figures/3Venn.svg', base_height = 8, base_width = 9)

#Additional bar graphs for visualizing feature annotations
...
#geom_text was creating file size issues, use the following as preparation for an alternative way to add n to graph

BGPrep <- Seperate_dedup_individ_bound %>%
  group_by(level, Sampler, Alignment_ID) %>%
  slice(1) %>%
  ungroup() %>%
  group_by(level, Sampler) %>%
  ...
  mutate(level = ifelse(level == 2, '2a/2b', level)) %>%
  mutate(level = ifelse(level > 3, '4/5', level)) %>%
  dplyr::summarize(n = n()) %>% #number of features per level
  mutate(h = n + 200) #value to be used for label height

#Create bar graph used in Figure 2
...
Feature_Levels_All <- Seperate_dedup_individ_bound %>%
  ...
  theme(axis.title.x = element_text(colour = "black", size = 25), 
        ...
        strip.text = element_text(colour = "black", size = 20), #adjust strip text if faceting#
        strip.background = element_rect(colour="black", fill="white")) +
  ggplot2::annotate("text", x = c(0.733, 1, 1.2667, 1.7333, 2, 2.2667, 2.7333, 3, 3.2667), y = c(BGPrep$h), label = c(BGPrep$n), size = 5) #add the n to each bar
  #geom_vline(xintercept = c(0.733, 1, 1.2667, 1.7333, 2, 2.2667, 2.7333, 3, 3.2667))
  #ggplot2::annotate(geom = "text", aes(y = n + 200, label = n), vjust = 0.5, size = 6)
  #geom_text(aes(y = n + 200, label = n), position = position_dodge(width = 0.8), vjust = 0.5, size = 6) # add n on each bar
Feature_Levels_All

save_plot(plot = Feature_Levels_All, 'Figures/ConfidenceBySamplerType.svg', base_height = 6, base_width = 6)

Feature_Levels_Combined <- Seperate_dedup_individ_bound %>%
  group_by(level, Alignment_ID) %>% #This time, just looking for unique features
  slice(1) %>%
  ungroup() %>%
  group_by(level) %>%
  mutate(level = as.character(level)) %>%
  ...
  geom_bar(width = 0.7, position = position_dodge(width = 0.8)) +
  labs(x = "Confidence Level", y = "Number of Features", fill = "Confidence Level") +
  ...
  theme(axis.title.x = element_text(colour = "black", size = 25), 
        ...
        strip.background = element_rect(colour="black", fill="white")) +
  geom_text(aes(y = n + 200, label = n), position = position_dodge(width = 0.8), vjust = 0.5, size = 6) # add n on each bar
Feature_Levels_Combined

save_plot(plot = Feature_Levels_Combined, 'Figures/Feature_Levels_Combined.svg', base_height = 9, base_width = 8)

####-----Calculating equivalent air volumes-----####

#Import air volumes from the AASs
...
#Create long versions of the within sampler type dedup tables#
...
##code calculates a concentration in PAU/L, where PAU represents arbitrary units for
##peak area, from each AAS and averages them into groups based on the deployment times##
##Notably, these are not ACTUALLY concentrations; they are just analogous when substituting PA for mass##
...
#Note how many features are in each sampler type
N_Features[,"Active"] = Get_N_Features(Dedup_A)
N_Features[,"PDMS"] = Get_N_Features(Dedup_D)
N_Features[,"GFF"] = Get_N_Features(Dedup_G)

#Note how many features have stable and unstable AAS measurements
N_Features[,"StableActive"] = Get_N_Features(AllActiveConc)
N_Features[,"UnstableActive"] = N_Features[,"Active"] - N_Features[,"StableActive"]

#Perform equivalent air volume calculations#

#Surface area of PDMS-Mesh#
...
#Number of features that equivalent air volume could be calculated for
N_Features[,"PDMS_Veqs"] = Get_N_Features(PDMS_Uptake)
N_Features[,"PDMS_NoActive"] = Get_N_Features(Dedup_D) - Get_N_Features(PDMS_Uptake)
...
EqAirVol_Filtering <- function(df){
  df2 <- df %>%
    ...
    dplyr::mutate(Range_Filt = ifelse(RelRange >= 40 & Days == 4,
                                      "Remove",
                                      ifelse(RelRange < 40 & Days == 4,
                                             "Keep",
                                             ifelse(RelRange >= 25,
                                                    "Remove",
                                                    "Keep")))) %>%
    #dplyr::mutate(Range_Filt = ifelse((RelRange >= 35 & Days == 4),
    #                                  "Remove",
    #                                  "Keep")) %>%
    #dplyr::mutate(Range_Filt = ifelse((RelRange >= 25 & Days > 4),
    #                                  "Remove",
...
RSA_GFF <- (0.238*0.238*3.14*2) #GFF-Mesh surface area#
...
#Again, get number of GFF-Mesh features with Veq#
N_Features[,"GFF_Veqs"] = Get_N_Features(GFF_Uptake)
N_Features[,"GFF_NoActive"] = Get_N_Features(Dedup_G) - Get_N_Features(GFF_Uptake)

#To get total unique features with Veq, comebine PDMS and GFF#
Both_PAS_Uptake <- bind_rows(PDMS_Uptake, GFF_Uptake)

N_Features[,"TotalPAS_Veqs"] = Get_N_Features(Both_PAS_Uptake)
N_Features[,"NoVeqs"] = N_Features[,"After_DetFreq_Dedup"] - N_Features[,"TotalPAS_Veqs"]

#Apply Veq filtering#
GFF_Uptake_stats <- EqAirVol_Filtering(GFF_Uptake)

#Get the number of features removed at this stage
N_Features[,"PDMS_VeqsPostFilter"] = Get_N_Features(PDMS_Uptake_stats)
N_Features[,"PDMS_VeqsFilteredOut"] = N_Features[,"PDMS_Veqs"] - N_Features[,"PDMS_VeqsPostFilter"]
N_Features[,"GFF_VeqsPostFilter"] = Get_N_Features(GFF_Uptake_stats)
N_Features[,"GFF_VeqsFilteredOut"] = N_Features[,"GFF_Veqs"] - N_Features[,"GFF_VeqsPostFilter"]

Both_PAS_Uptake_stats <- bind_rows(PDMS_Uptake_stats, GFF_Uptake_stats)

N_Features[,"Total_VeqsPostFilter"] = Get_N_Features(Both_PAS_Uptake_stats)
N_Features[,"Total_VeqsFilteredOut"] = N_Features[,"TotalPAS_Veqs"] - N_Features[,"Total_VeqsPostFilter"]
...
#Used to ultimately create document of uptake curves that can be quickly inspected for QA/QC and workflow development
#Uptake curves shown in manuscript are created later with better formatting
...
Box_Plot <- function(df, x, y, fill_by, x_title, y_title, max_val_from, showlegend = FALSE,
  ...
  dfplot <- df%>%
    ...
    geom_boxplot(show.legend = showlegend, outlier.shape = NA)+ #box plot; legend is unnecessary#
    #geom_dotplot(binaxis = 'y', stackdir = 'up', method = "histodot", dotsize = 7, binwidth = 0.01, alpha = 0.8) +
    geom_jitter(shape = 21, size = 2, alpha = 0.8, show.legend = FALSE) + #add individual data points
    theme_bw()+ #aesthetics#
...
N_Features[,"PDMS_URs"] = Get_N_Features(PDMS_UR_fullfilt)
N_Features[,"GFF_URs"] = Get_N_Features(GFF_UR_fullfilt)
N_Features[,"Total_URs"] = Get_N_Features(NewFilt_Both_PAS)

N_Features[,"Uncalibrated_AAS"] = Get_N_Features(Dedup_A) - Get_N_Features(NewFilt_Both_PAS)
N_Features[,"Uncalibrated_PDMS"] = Get_N_Features(Dedup_D) - Get_N_Features(PDMS_UR_fullfilt)
N_Features[,"Uncalibrated_GFF"] = Get_N_Features(Dedup_G) - Get_N_Features(GFF_UR_fullfilt)
...
NewFilt_Both_PAS_IQR$Sorbent <- factor(NewFilt_Both_PAS_IQR$Sorbent,
                                       levels = c("PDMS", "GFF"),
                                       labels = c("PDMS-Mesh", "GFF-Mesh")) #reorder layers#

N_Features[,"PDMS_AfterOutlier"] = Get_N_Features(PDMS_UR_fullfilt_IQR)
N_Features[,"PDMS_NumOutlier"] = N_Features[,"PDMS_URs"] - N_Features[,"PDMS_AfterOutlier"]
N_Features[,"GFF_AfterOutlier"] = Get_N_Features(GFF_UR_fullfilt_IQR)
N_Features[,"GFF_NumOutlier"] = N_Features[,"GFF_URs"] - N_Features[,"GFF_AfterOutlier"]

#Plot IQR corrections, this is the box plot for Figure 3#
NewFilt_IQR_BP_Rates <- Box_Plot(NewFilt_Both_PAS_IQR, Sorbent, Uptake_Rate, Sorbent, "Sampler Type",
                                 expression(paste("Uptake Rate (m"^"3"*" day"^"-1"*" dm"^"-2"*")")),
...
Check1321 <- feature_table_clean %>%
  filter(Alignment_ID == 1321)

Check2145 <- feature_table_clean %>%
  filter(Alignment_ID == 2145)
...
bad_smiles2 <- read.delim(file = "OPERA 09/OPERA_newest_fails.txt") %>%
  mutate(Alignment_ID = as.character(Alignment_ID))
bad_smiles3 <- read.delim(file = "OPERA 09/Additional_Bad_SMILES.txt") %>%
  mutate(Alignment_ID = as.character(Alignment_ID))

bad_smiles <- bind_rows(bad_smiles1, bad_smiles2, bad_smiles3)

N_Features[,"bad_smiles"] = Get_N_Features(bad_smiles)
...
#Import KOAs missed by previous runs
OPERA_added <- read_csv("OPERA 09/09 OPERA Results/OPERA_additional.csv", col_names = T) %>%
  select(-`...1`)

#Combine all KOAs back together
OPERA_all <- bind_rows(OPERA_all, OPERA_added)
...
missing_opera <- merge(OPERA_all, opera_export, by = c('Alignment_ID', '2D_INCHIKEY'), all = T) %>% #had failed smiles
  ...
  group_by(Alignment_ID) %>% #remove duplicates
  slice(1) %>%
  ungroup() %>%
  select(-LogKOA_pred)
...
#Having obtained KOA with missing_opera, import those results, then export again so they can be re-imported above in future runs
#OPERA_additional <- read_csv("OPERA 09/09 OPERA Results/OPERA_SMILES_Additional3-smi_OPERA2.9Pred.csv", col_names = T) %>%
#  dplyr::rename(Alignment_ID = MoleculeID) %>% #id was 2D inchikey
#  merge(., missing_opera, by = c('Alignment_ID')) %>% #merge back with rest of information for each feature
#  subset(select = c(`2D_INCHIKEY`, LogKOA_pred, Alignment_ID)) %>% #get only infromation needed
#  group_by(Alignment_ID) %>% #prevent duplicates when later merging (use alignment id in case some don't have 2d inchikey)
#  slice(1) %>% #prevent duplicates when later merging
#  ungroup()

#DF_to_CSV(OPERA_additional)
...
koa <- All_Features_KOAadded %>%
  filter(!is.na(LogKOA_pred))

N_Features[,"KOAs"] = Get_N_Features(koa)
#N_Features[,"MissingKOAs"] = Get_N_Features(Seperate_dedup_individ_bound_level32) - N_Features[,"KOAs"]
...
#Special version for specific plots
All_Features_KOAadded_ForPlot <- All_Features_KOAadded

All_Features_KOAadded_ForPlot$Sampler <- factor(All_Features_KOAadded_ForPlot$Sampler,
                                        levels = c("PDMS", "GFF", "Active"),
                                        labels = c("PDMS-Mesh", "GFF-Mesh", "Active")) #reorder layers#

KOAs_for_Plots <- all_koas_filtered %>%
  select(c(Alignment_ID, LogKOA_pred, Average_Mz, Sampler))

#Distribution of KOAs used in Figure 5

Detection_Comparison <- All_Features_KOAadded_ForPlot %>%
...
#Distribution of m/z used in Figure 5

Detection_Comparison_MZ <- All_Features_KOAadded_ForPlot %>%
  filter(Sampler != "Active") %>%
  group_by(Sampler) %>%
  dplyr::mutate(n = n()) %>%
  ungroup() %>%
...
#For constructing other plots, create a column in the KOA table that will identify if a feature is calibrated
...
#uptake capacity analysis#

#Calculate Kpdms-air based on equations from Tromp et al.
Kpdms_air <- All_Features_KOAadded %>%
  filter(Sampler == "PDMS" & !is.na(LogKOA_pred)) %>%
  mutate(LogKpdms_air = LogKOA_pred*0.778+0.813) %>%
  mutate(Kpdms_air = 10^LogKpdms_air)

#Match calibrated features with the Kpdms-air
Kpdms_air_URs <- merge(Kpdms_air, PDMS_UR_fullfilt_IQR, by = "Alignment_ID")

#Sampler dimensions
PDMS_V = (5.08/100)*(5.08/100)*(0.16/100) #in m^3

#3. get constants for equations
Ct25 = 0.75
Ct95 = 0.05

#Generic uptake rate for PDMS, remove surface area normalization for units of just m3/day
RsPDMS = 1.16*RSA_PDMS

#Convert specific uptake rates to units of m3/day
Kpdms_air_URs[,"RsPDMS_Specific"] = Kpdms_air_URs[,"Uptake_Rate"]*RSA_PDMS

#Using equation from OKeme et al. 2016 on Kpdms-air, calculate time to curvilinear and equilibrium for each feature
Kpdms_air[,'t25_pdms'] = -log(Ct25)*(((PDMS_V*(Kpdms_air$Kpdms_air)))/RsPDMS)
Kpdms_air[,'t95_pdms'] = -log(Ct95)*(((PDMS_V*(Kpdms_air$Kpdms_air)))/RsPDMS)

#Repeat for calibrated features specifically with generic uptake rate
Kpdms_air_URs[,'t25_pdms'] = -log(Ct25)*(((PDMS_V*(Kpdms_air_URs$Kpdms_air)))/RsPDMS)
Kpdms_air_URs[,'t95_pdms'] = -log(Ct95)*(((PDMS_V*(Kpdms_air_URs$Kpdms_air)))/RsPDMS)

#Repeat based on specific uptake rate
Kpdms_air_URs[,'t25_pdms_spec'] = -log(Ct25)*(((PDMS_V*(Kpdms_air_URs$Kpdms_air)))/Kpdms_air_URs$RsPDMS_Specific)
Kpdms_air_URs[,'t95_pdms_spec'] = -log(Ct95)*(((PDMS_V*(Kpdms_air_URs$Kpdms_air)))/Kpdms_air_URs$RsPDMS_Specific)

#Determine the log Kpdms-air thresholds for curvilinear and equilibration
LogKpdms_for_CL = log10((20*RsPDMS)/(-log(Ct25)*(PDMS_V)))
LogKpdms_for_Eq = log10((20*RsPDMS)/(-log(Ct95)*(PDMS_V)))

#Finally determine the minimum log KOA a feature can have before entering curvilinear and equilibrium, based on modelled predictions
KOA_for_CL = (LogKpdms_for_CL - 0.813)/0.778
KOA_for_Eq = (LogKpdms_for_Eq - 0.813)/0.778

#Create lists of uncalibrated features with KOA and m/z values
NoCalibrateKOA_PDMS <- Kpdms_air %>%
  filter(Alignment_ID %in% PDMS_Uptake$Alignment_ID) %>%
  filter(Alignment_ID %in% PDMS_UR_fullfilt$Alignment_ID == FALSE)

NoCalibrateMZ_PDMS <- All_Features %>%
  filter(Sampler == "PDMS" & Alignment_ID %in% PDMS_Uptake$Alignment_ID) %>%
  filter(Alignment_ID %in% PDMS_UR_fullfilt$Alignment_ID == FALSE)

NoCalibrateKOA_GFF <- All_Features_KOAadded %>%
  filter(Sampler == "GFF" & !is.na(LogKOA_pred) & Alignment_ID %in% GFF_Uptake$Alignment_ID) %>%
  filter(Alignment_ID %in% GFF_UR_fullfilt$Alignment_ID == FALSE)

NoCalibrateMZ_GFF <- All_Features %>%
  filter(Sampler == "GFF" & Alignment_ID %in% GFF_Uptake$Alignment_ID) %>%
  filter(Alignment_ID %in% GFF_UR_fullfilt$Alignment_ID == FALSE)

#Create density plot used for Figure S22

#Colours for figure
KOA_Filter_PDMS_Colours <- c("#7FC97F", "grey")

#Distribution of KOAs for uncalibrated features on PDMS
CompareKOAFilter_PDMS <- All_Features_KOAadded %>%
  filter(Sampler == "PDMS" & !is.na(LogKOA_pred)) %>%
  filter(Alignment_ID %in% PDMS_UR_fullfilt$Alignment_ID | Alignment_ID %in% NoCalibrateKOA_PDMS$Alignment_ID) %>%
  mutate(Has_Good_UR = ifelse(grepl("No", Has_Good_UR),
                              "PDMS Uncalibrated",
                              "PDMS Calibrated")) %>%
  ggplot(aes(x = LogKOA_pred, fill = Has_Good_UR)) +
  scale_fill_manual(values = KOA_Filter_PDMS_Colours) +
  ggplot2::annotate(geom = "rect", xmin = 0, xmax = KOA_for_Eq, ymin = 0, ymax = 0.4, color="white", fill="orange", alpha = 0.1) +
  ggplot2::annotate(geom = "rect", xmin = KOA_for_Eq, xmax = KOA_for_CL, ymin = 0, ymax = 0.4, color="white", fill="white", alpha = 0.1) +
  ggplot2::annotate(geom = "rect", xmin = KOA_for_CL, xmax = 14.2, ymin = 0, ymax = 0.4, color="white", fill="pink", alpha = 0.2) +
  ggplot2::annotate(geom = "text", x = (0 + KOA_for_Eq)/2, y = 0.3, label = str_wrap("Estimated Equilibrium (PDMS)", 15), fontface = "italic", size = 4) +
  ggplot2::annotate(geom = "text", x = (KOA_for_CL + KOA_for_Eq)/2, y = 0.3, label = str_wrap("Estimated Curvilinear (PDMS)", 15), fontface = "italic", size = 4) +
  ggplot2::annotate(geom = "text", x = (KOA_for_CL + 14.2)/2, y = 0.3, label = str_wrap("Estimated Linear (PDMS)", 15), fontface = "italic", size = 4) +
  geom_density(alpha = 0.6)+ #use scatterplot#
  scale_y_continuous(expand = c(0, 0)) +
  scale_x_continuous(breaks = seq(0, 14, by = 1), expand = c(0, 0), limits = c(0, 14.2)) +
  theme_bw()+ #aesthetics#
  ...
  theme(axis.title.x = element_text(size = 20), 
        ...
        panel.border = element_blank(), #remove border#
        strip.background = element_rect(colour="black", fill="white")) #black and white background#
CompareKOAFilter_PDMS

KOA_Filter_GFF_Colours <- c("#BEAED4", "grey")

#Repeat for GFF
CompareKOAFilter_GFF <- All_Features_KOAadded %>%
  filter(Sampler == "GFF" & !is.na(LogKOA_pred)) %>%
  filter(Alignment_ID %in% GFF_UR_fullfilt$Alignment_ID | Alignment_ID %in% NoCalibrateKOA_GFF$Alignment_ID) %>%
  mutate(Has_Good_UR = ifelse(grepl("No", Has_Good_UR),
                              "GFF Uncalibrated",
                              "GFF Calibrated")) %>%
  ggplot(aes(x = LogKOA_pred, fill = Has_Good_UR)) +
  scale_fill_manual(values = KOA_Filter_GFF_Colours) +
  #ggplot2::annotate(geom = "rect", xmin = 0, xmax = KOA_for_Eq, ymin = 0, ymax = 0.4, color="white", fill="orange", alpha = 0.1) +
  #ggplot2::annotate(geom = "rect", xmin = KOA_for_Eq, xmax = KOA_for_CL, ymin = 0, ymax = 0.4, color="white", fill="white", alpha = 0.1) +
  #ggplot2::annotate(geom = "rect", xmin = KOA_for_CL, xmax = 14.2, ymin = 0, ymax = 0.4, color="white", fill="pink", alpha = 0.2) +
  #ggplot2::annotate(geom = "text", x = (0 + KOA_for_Eq)/2, y = 0.3, label = str_wrap("Estimated Equilibrium (PDMS)", 15), fontface = "italic", size = 4) +
  ...
  #ggplot2::annotate(geom = "text", x = (KOA_for_CL + 14.2)/2, y = 0.3, label = str_wrap("Estimated Linear (PDMS)", 15), fontface = "italic", size = 4) +
  geom_density(alpha = 0.6)+ #use scatterplot#
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.28)) +
  scale_x_continuous(breaks = seq(0, 14, by = 1), expand = c(0, 0), limits = c(0, 14.2)) +
  theme_bw()+ #aesthetics#
  labs(x = expression(paste("Predicted log"[10]*" K"[OA]*"")),
       y = "Density",
...
StudyURs_imp <- read.csv("C://Users//scrol//OneDrive - McMaster University//Joseph Okeme's files - Alyss//New passives calibration//Manuscript for PAS//PaperURs.csv")%>%
  pivot_longer(cols = 1:13, names_to = "Study", values_to = "Uptake_Rate")%>%
                          ...
                          ifelse(grepl("_db", Study),
                                 ...
                                 ifelse(grepl("_sb", Study) | grepl("PUF", Study),
                                        ...
                                        ifelse(grepl("_PDMS", Study),
                                               "Unsheltered PDMS sheet",
                                               ifelse(grepl("_XAD", Study),
                                                      "XAD-Mesh",
                                                      ifelse(grepl("_spf", Study),
                                                             "Sheltered PDMS foam",
                                                             "Unsheltered PDMS foam"))))))) %>%
  mutate(Sampler = str_wrap(Sampler, 12)) %>%
  mutate(Study = gsub("_", " ", Study))%>%
  mutate(Study = gsub("al", "al.", Study))%>%
  mutate(Study = gsub(" uspf", "", Study))%>%
  mutate(Study = gsub(" db", "", Study))%>%
  mutate(Study = gsub(" sb", "", Study))%>%
  mutate(Study = gsub(" PDMS", "", Study))%>%
  mutate(Study = gsub(" XAD", "", Study))%>%
  mutate(Study = gsub(" spf", "", Study))%>%
...
Comparing_Study_URs$Sampler = factor(Comparing_Study_URs$Sampler,
                                     levels = c("PDMS-Mesh", "GFF-Mesh",
                                                "Double bowl\nPUF", "Sheltered\nPDMS foam",
                                                "Unsheltered\nPDMS foam",
                                                "Single bowl\nPUF",
                                                "Unsheltered\nPDMS sheet",
                                                "XAD-Mesh", "Unsheltered\nPUF"),
                                     labels = c("PDMS-Mesh", "GFF-Mesh",
                                                "Double bowl\nPUF", "Sheltered\nPDMS foam",
                                                "Unsheltered\nPDMS foam",
                                                "Single bowl\nPUF",
                                                "Unsheltered\nPDMS sheet",
...
#Calculate average uptake rates across studies based on sampler type

PAS_Type_Gen_PDMS <- Comparing_Study_URs %>%
  filter(grepl("PDMS", Sampler) & Study != "Present Work") %>%
  dplyr::summarize(mean = mean(Uptake_Rate),
                   stdev = sd(Uptake_Rate))

PAS_Type_Gen_PUF <- Comparing_Study_URs %>%
  filter(grepl("PUF", Sampler) & !grepl("Double", Sampler)) %>%
  dplyr::summarize(mean = mean(Uptake_Rate),
                   stdev = sd(Uptake_Rate))

PAS_Type_Gen_DB <- Comparing_Study_URs %>%
  filter(grepl("Double", Sampler)) %>%
  dplyr::summarize(mean = mean(Uptake_Rate),
                   stdev = sd(Uptake_Rate))

PAS_Type_Gen_XAD <- Comparing_Study_URs %>%
  filter(grepl("XAD", Sampler)) %>%
  dplyr::summarize(mean = mean(Uptake_Rate),
                   stdev = sd(Uptake_Rate))

#Create Figure 4 comparing uptake rates across literature

StudyURPalette = c(brewer.pal(8, "Accent"), "#ca24d3")

BP_Comparing_Study_URs <- Comparing_Study_URs %>%
  ggplot(aes(x = Study, y = Uptake_Rate, fill = Sampler))+
  geom_boxplot(position = position_dodge(1), outlier.shape = NA)+ #box plot; legend is unnecessary#
  #geom_violin() +
  geom_jitter(shape = 21, size = 2, position = position_jitterdodge(dodge.width = 1), alpha = 0.8, show.legend = FALSE) +
  #geom_dotplot(binaxis='y', stackdir='center', position = position_dodge(1), dotsize=5, binwidth = 0.01, alpha = 0.8) +
  scale_fill_manual(values = StudyURPalette)+
...
save_plot(plot = BP_Comparing_Study_URs, 'Figures/Figure_4.svg', base_width = 15, base_height = 8)

#-------visualize classification-------

#filter for level 2 and 3 compounds
...
#filter for confident classifications
classes1 <- classes %>%
...
#get number of each classification remaining after filtering to put in manuscript text
...
N_Features[,"Classified"] = length(number_compounds_classified)
N_Features[,"LostWhenClassified"] = length(unique(Seperate_dedup_individ_bound_level32$Alignment_ID)) - length(number_compounds_classified)
...
#Special version for a figure
PAS_PeakAreas_F6a <- PAS_PeakAreas
PAS_PeakAreas_F6a$Sorbent <- factor(PAS_PeakAreas_F6a$Sorbent,
                                levels = c("PDMS", "GFF"),
                                labels = c("PDMS-Mesh", "GFF-Mesh")) #reorder layers#

#Create panel B of Figure 6

Superclass_With_PeakArea_Both <- PAS_PeakAreas_F6a%>%
  ...
  labs(x = element_blank(), fill  = str_wrap("Total Normalized Peak Area", 20))+ #axis titles#
  theme(#axis.title.x = element_text(size = 20), 
    #axis.title.y = element_text(size = 20), #adjust text size for axis titles#
    axis.text.x = element_text(colour = "black", size = 15),
...
#Preparation for creating panel a of Figure 6
...
Superclass_With_RDF_Both_Prep <- PAS_PeakAreas%>%
  merge(., NumRDF_Superclass, by = c("Sorbent", "Superclass"), all = TRUE)

Superclass_With_RDF_Both_Prep$Sorbent <- factor(Superclass_With_RDF_Both_Prep$Sorbent,
                                    levels = c("PDMS", "GFF"),
                                    labels = c("PDMS-Mesh", "GFF-Mesh")) #reorder layers#

#plot of superclasses with proportion of features with uptake rates for panel a of Figure 6
Superclass_With_RDF_Both <- Superclass_With_RDF_Both_Prep%>%
  ...
  coord_radial(start = -0.04, expand = TRUE, clip = "off")+
  facet_wrap(~Sorbent, ncol = 1, nrow = 2)+
  labs(x = element_blank(), fill = str_wrap("Proportion of Calibrated Features (%)", 20))+ #axis titles#
  theme(#axis.title.x = element_text(size = 20), 
    #axis.title.y = element_text(size = 20), #adjust text size for axis titles#
    axis.text.x = element_text(colour = "black", size = 15),
    #axis.text.y = element_text(colour = "black", size = 7), #adjust axis text size and colour#
...
####Representative uptake curves for Figure 3####

Rep_Curve_List <- c(1367, 1814, 4516, 4841) #overlapping features between sorbents

#Table containing the Veq for the representative features
Rep_Curves <- Both_PAS %>%
  filter(Alignment_ID %in% Rep_Curve_List)

Rep_Curves$PAS_Type = factor(Rep_Curves$PAS_Type,
                             levels = c("PDMS", "GFF"),
                             labels = c("PDMS-Mesh", "GFF-Mesh"))
...
#Plot uptake curves for representative features
Rep_Curves_Plot <- Rep_Curves %>%
  ggplot(aes(x = Days, y = Vol, colour = PAS_Type, fill = PAS_Type, shape = PAS_Type)) +
  geom_point(size = 4, alpha = 0.5)+ #use scatterplot#
  ...
  labs(x = "Deployment time (days)", y = expression(paste("Equivalent air volume (m"^3*")")), fill = "PAS Sorbent",
...
#plot uptake rates from each sorbent against one another, used in Figure 3
PDMSvGFF <- URs_on_both %>%
  ...
  labs(x = expression(paste("Rate GFF-Mesh (m"^"3"*" day"^"-1"*" dm"^"-2"*")")),
       y = expression(paste("Rate PDMS-Mesh (m"^"3"*" day"^"-1"*" dm"^"-2"*")"))) +
...
#uptake rates vs KOA, used in Figure 5
#Includes shaded regions where uptake is linear, curvilinear, and equilibrium based on modelling for PDMS

KOAvUR_Prep <- NewFilt_Both_PAS
KOAvUR_Prep$Sorbent = factor(KOAvUR_Prep$Sorbent,
                             levels = c("PDMS", "GFF"),
                             labels = c("PDMS-Mesh", "GFF-Mesh"))

KOAvUR <- KOAvUR_Prep %>%
  dplyr::rename(Sampler = Sorbent) %>%
  merge(., All_Features_KOAadded_ForPlot, by = c("Alignment_ID", "Sampler")) %>%
  filter(!is.na(LogKOA_pred)) %>%
  ...
  #geom_hline(yintercept = (Generic_GFF - 2*Generic_GFF_SD), linetype = "dashed", colour = "red", linewidth = 1) +
  ggplot2::annotate(geom = "rect", xmin = 2.8, xmax = KOA_for_Eq, ymin = -0.3, ymax = 10.5, color="white", fill="orange", alpha = 0.1) +
  ggplot2::annotate(geom = "rect", xmin = KOA_for_Eq, xmax = KOA_for_CL, ymin = -0.3, ymax = 10.5, color="white", fill="white", alpha = 0.1) +
  ggplot2::annotate(geom = "rect", xmin = KOA_for_CL, xmax = 11, ymin = -0.3, ymax = 10.5, color="white", fill="pink", alpha = 0.2) +
  ggplot2::annotate(geom = "text", x = (2.8 + KOA_for_Eq)/2, y = 9, label = str_wrap("Estimated Equilibrium (PDMS)", 15), fontface = "italic", size = 5) +
  ggplot2::annotate(geom = "text", x = (KOA_for_CL + KOA_for_Eq)/2, y = 9, label = str_wrap("Estimated Curvilinear (PDMS)", 15), fontface = "italic", size = 5) +
  ggplot2::annotate(geom = "text", x = (KOA_for_CL + 11)/2, y = 9, label = str_wrap("Estimated Linear (PDMS)", 15), fontface = "italic", size = 5) + #These lines highlight the uptake regions
  #geom_vline(xintercept = KOA_for_Eq, colour = "red") +
  #geom_vline(xintercept = KOA_for_CL, colour = "yellow") +
  geom_point(size = 5, alpha = 0.8)+ #use scatterplot#
  scale_y_continuous(breaks = seq(0, 10, by = 1), expand = c(0, 0), limits = c(-0.3, 10.5)) +
  scale_x_continuous(breaks = seq(3, 11, by = 1), expand = c(0, 0), limits = c(2.8, 11)) +
  theme_bw()+ #aesthetics#
  labs(x = expression(paste("Predicted log"[10]*" K"[OA]*"")),
...
save_plot(plot = KOAvUR, "Figures/KOAvUR.svg", base_height = 8, base_width = 15)

#uptake rate vs m/z, used for Figure 5
MZvUR_Prep <- NewFilt_Both_PAS
MZvUR_Prep$Sorbent = factor(MZvUR_Prep$Sorbent,
                             levels = c("PDMS", "GFF"),
                             labels = c("PDMS-Mesh", "GFF-Mesh"))
...
MZvUR <- MZvUR_Prep %>%
  ...
  scale_colour_brewer(palette = "Accent") +
  scale_y_continuous(breaks = seq(0, 10, by = 1), expand = c(0, 0), limits = c(-0.3, 10.5)) +
  scale_x_continuous(breaks = seq(100, 500, by = 50), expand = c(0, 0), limits = c(90, 510)) +
  theme_bw()+ #aesthetics#
...
URs_and_PChem <- NewFilt_Both_PAS %>%
  ...
  mutate(LogKOA_pred = ifelse(is.na(LogKOA_pred),
                              0,
                              LogKOA_pred)) %>%
  filter(LogKOA_pred <8.5 & LogKOA_pred > 0) %>%
  group_by(Sampler) %>%
  dplyr::mutate(Mean = mean(Uptake_Rate),
                STDEV = sd(Uptake_Rate)) %>%
...
#Repeats the workflow used for Veq, just without normalizing to AAS measurements
...
PDMS_AllPeakAreas <- uPDMS %>%
  ...
  dplyr::rename(Vol = Area) %>% #Temporary rename so the EqAirVol function can be used
  EqAirVol_Filtering() %>%
  dplyr::rename(Area = Vol) #Change name back to Area

Reg_PDMS_Areas <- PDMS_AllPeakAreas %>%
  filter(Alignment_ID %in% Dedup_A$Alignment_ID == FALSE) %>%
  group_by(Alignment_ID)%>% #cluster by Alignment_ID#
...
N_Features[,"PA_Curves"] = Get_N_Features(NumPAFeaturesUnique)

PDMS_Areas_stats$Alignment_ID %in% PDMS_stats$Alignment_ID
GFF_Areas_stats$Alignment_ID %in% GFF_stats$Alignment_ID

#Representative uptake curves for peak areas, Figure S21#
...
Rep_PACurves_Plot <- Rep_PACurves %>%
  ggplot(aes(x = Days, y = Area, colour = Sampler, fill = Sampler, shape = Sampler)) +
  geom_point(size = 4, alpha = 0.5)+ #use scatterplot#
  #scale_y_continuous(breaks = seq(0, 25, by = 5), expand = c(0, 0), limits = c(0, 25)) +
  scale_x_continuous(breaks = seq(0, 20, by = 4), expand = c(0, 0, 0.1, 0), limits = c(0, 20)) +
  scale_color_brewer(palette = "Accent") +
  ...
  labs(x = "Deployment time (days)", y = "Peak Area (PAU)",
       colour = "PAS Sorbent", fill = "PAS Sorbent", shape = "PAS Sorbent") +
  geom_smooth(method = lm, formula = y ~ 0 + x, se = FALSE)+ #add linear regression#
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank())+ #remove gridlines#
  theme(axis.title.x = element_text(size = 20), 
        axis.title.y = element_text(size = 20), #adjust text size for axis titles#
...
save_plot(plot = Rep_PACurves_Plot, "Figures/RepCurves.svg", base_height = 8, base_width = 15)

####-----Finalized Figures for Manuscript-----####

#Former version of Figure 2 not used, made of Venn diagrams

VenFig2 <- ggarrange(sampler_type_ven, level2_ven, level3_ven, level45_ven,
                     labels = c("A", "B", "C", "D"),
                     font.label = list(colour = "black", size = 35))
VenFig2

save_plot(plot = VenFig2, "Figures/VenFig2.svg", base_height = 16.91, base_width = 15)

#Figure 2; ultimately constructed in PowerPoint instead due to formatting difficulties

Figure2 <- ggarrange(sampler_type_ven, Feature_Levels_All,
                     labels = c("A", "B"), nrow = 1, ncol = 2, widths = c(1,2), hjust = c(-4, -4), align = "hv",
...
preFigure3 <- ggarrange(NewFilt_IQR_BP_Rates, PDMSvGFF,
                        ...
                        font.label = list(colour = "black", size = 25))
preFigure3 #preparing part of figure 3

Figure3 <- ggarrange(Rep_Curves_Plot, preFigure3,
                     labels = c("A", ""), hjust = c(-3.7, 0), vjust = c(2, NA),
                     nrow = 2,
                     font.label = list(colour = "black", size = 25))
Figure3 #figure 3

save_plot(plot = Figure3, "Figures/Figure_3.svg", base_height = 10, base_width = 10)

Figure5 <- ggarrange(KOAvUR, MZvUR, Detection_Comparison, Detection_Comparison_MZ,
                     labels = c("A", "B", "C", "D"),
                     nrow = 2, ncol = 2, hjust = -3.5, align = "hv",
                     font.label = list(colour = "black", size = 35),
...
FigureSKoaFilt <- ggarrange(CompareKOAFilter_PDMS, CompareKOAFilter_GFF,
                     labels = c("A", "B"),
                     nrow = 2, ncol = 1, hjust = -3.5, align = "hv",
                     font.label = list(colour = "black", size = 35))
FigureSKoaFilt

save_plot(plot = FigureSKoaFilt, "Figures/Figure_SKoaFilt.svg", base_height = 10, base_width = 15)

####-----Pulling out numbers used in manuscript-----####
...
#make a table with uptake rates and pchem for SI; note some features will not have uptake rates carried into this, add them manually
...
#important numbers for use in manuscript, largely replaced by N_Features
...
####-----All Uptake Curves for SI-----####

#Create subsets of <=16 curves
...
PDMS_CurveList5 <- PDMS_UR_fullfilt %>%
  slice(65:67)
...
PDMS_Curves1 <- plotPDMS_filter %>%
  ...
  ggplot(aes(x = Days, y = Vol, colour = Sampler, fill = Sampler, shape = Sampler)) +
  geom_point(size = 4, show.legend = FALSE, alpha = 0.5)+ #use scatterplot#
  scale_colour_manual(values = PDMS_Colours) +
  scale_fill_manual(values = PDMS_Colours) +
...
PDMS_Curves5 <- plotPDMS_filter %>%
  filter(Alignment_ID %in% PDMS_CurveList5$Alignment_ID) %>%
  ggplot(aes(x = Days, y = Vol, colour = Sampler, fill = Sampler, shape = Sampler)) +
  geom_point(size = 4, show.legend = FALSE, alpha = 0.5)+ #use scatterplot#
  scale_colour_manual(values = PDMS_Colours) +
  scale_fill_manual(values = PDMS_Colours) +
  ...
  scale_x_continuous(breaks = seq(0, 20, by = 4), expand = c(0, 0, 0.1, 0), limits = c(0, 20)) +
  theme_bw()+ #aesthetics#
  facet_wrap(~Alignment_ID, nrow = 1, ncol = 3, scales = "free")+
  labs(x = "Deployment time (days)", y = expression(paste("Equivalent air volume (m"^3*")"))) +
  geom_smooth(method = lm, formula = y ~ 0 + x, se = FALSE, show.legend = FALSE)+ #add linear regression#
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank())+ #remove gridlines#
  theme(axis.title.x = element_text(size = 20), 
...
GFF_Curves1 <- plotGFF_filter %>%
  ...
  ggplot(aes(x = Days, y = Vol, colour = GFF_Colours, fill = GFF_Colours, shape = Sampler)) +
  geom_point(size = 4, show.legend = FALSE, alpha = 0.5)+ #use scatterplot#
  scale_colour_manual(values = GFF_Colours) +
  scale_fill_manual(values = GFF_Colours) +
...
save_plot(plot = PDMS_Curves4, "Figures/PDMS_Curves4.svg", base_height = 16, base_width = 16)
save_plot(plot = PDMS_Curves5, "Figures/PDMS_Curves5.svg", base_height = 4, base_width = 12)

save_plot(plot = GFF_Curves1, "Figures/GFF_Curves1.svg", base_height = 16, base_width = 16)
save_plot(plot = GFF_Curves2, "Figures/GFF_Curves2.svg", base_height = 16, base_width = 16)
save_plot(plot = GFF_Curves3, "Figures/GFF_Curves3.svg", base_height = 16, base_width = 16)
